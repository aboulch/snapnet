    ...
    def train(self,
        ...
        with tf.Graph().as_default() as g:
            ...
            with tf.variable_scope(variable_scope) as scope:
                ...
                # create model
                deconv_net, net = self.model_function(images, label_nbr_original_mode)

                # create saver
                saver1 = tf.train.Saver([v for v in tf.global_variables() if variable_scope in v.name])

                # add a convolution
                deconv_net.append(convolution2d_transpose(deconv_net[-2], label_nbr, stride=1))
            ...
            ckpt = tf.train.get_checkpoint_state(net_weights_init)
